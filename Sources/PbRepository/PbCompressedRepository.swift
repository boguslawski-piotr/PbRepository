...
open class PbCompressedRepository : PbRepositoryDecoratorBase, PbRepository, PbRepositoryAsync
{
    ...
    open func retrieveAsync<T>(itemOf type: T.Type, from name: String) async throws -> T? where T : Decodable {
    ...
    open func storeAsync<T>(sequence: T, to name: String) async throws where T : Sequence, T.Element : Encodable {
    ...
    open func retrieve<T>(sequenceOf type: T.Type, from name: String) throws -> ThrowingStream<T, Error>? where T : Decodable {
        guard var compressedDataIterator = try rF!.retrieve(sequenceOf: Data.self, from: name)?.makeIterator() else { return nil }
    ...
    open func retrieveAsync<T>(sequenceOf type: T.Type, from name: String) async throws -> AsyncThrowingStream<T, Error>? where T : Decodable {
        guard var compressedDataIterator = try await rFA?.retrieveAsync(sequenceOf: Data.self, from: name)?.makeAsyncIterator() else { return nil }
